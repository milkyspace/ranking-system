...
use Ranking\RankingConditions\First\Multiplying\EnFirstInEuropeCondition;
use Ranking\RankingConditions\First\Multiplying\EnFirstInRussiaCondition;
...
/**
 * Class FirstEnRanking
 * @package Ranking\RankingSystems
 */
final class FirstEnRanking extends Condition implements IRanking
{
    /** @var $id int */
    protected $id;
    ...
    public function calcRanking(?IRankingCommonData $rankCommonData = null, ?RankingLogger $logger = null): int
    {
        ...
        $ranking->setMultiply();
        $ranking = new EnFirstInRussiaCondition($ranking, $build);
        $ranking->setMultiply();
        $ranking = new EnFirstInEuropeCondition($ranking, $build);
        $ranking->setMultiply();

        // Композицией добавляем другую систему ранкинга: ранкинг тревел-эксперта
        $ranking = new SecondEnRanking($ranking, $build);
